...
public sealed class PlacesServiceFavouriteTests
{
    private static PlacesService NewServiceWithThreePlaces(out Place a, out Place b, out Place c)
    {
        ...
        service.TryAdd("A", PlaceType.Folder, TestPaths.Folder(@"A"), out var placeA, out _);
        service.TryAdd("B", PlaceType.Folder, TestPaths.Folder(@"B"), out var placeB, out _);
        service.TryAdd("C", PlaceType.Folder, TestPaths.Folder(@"C"), out var placeC, out _);
        a = placeA!;
        b = placeB!;
        c = placeC!;
        return service;
