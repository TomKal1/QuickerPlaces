...
public sealed class PlacesServiceValidationTests
{
    ...
    public void DuplicateAlias_IsRejectedCaseInsensitively()
    {
        ...
        Assert.True(service.TryAdd("Docs", PlaceType.Folder, TestPaths.Folder(@"Docs"), out _, out _).Success);

        var result = service.TryAdd("docs", PlaceType.Folder, TestPaths.Folder(@"OtherDocs"), out var created, out _);
    ...
    public void SameResourceString_UnderDifferentType_IsAllowed()
    {
        ...
        // A fully-qualified path is, perhaps surprisingly, also a valid
        // absolute Uri (System.Uri treats "C:\..." on Windows, and "/..."
        // on Linux, as an implicit file:// URI) — so this one literal
        // legitimately passes both
        // ValidateFolderFormat and ValidateUrlFormat, letting the same
        // resource string be used for both PlaceTypes in this test.
        var resource = TestPaths.Folder(@"Shared\Data");
    ...
    public void EditingAPlace_DoesNotCollideWithItself()
    {
        ...
        var resourceResult = service.TryEditResource(place, TestPaths.Folder(@"Docs"), out _);
