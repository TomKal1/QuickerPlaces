...
public sealed class FilePlacesStorageTests
{
    ...
    public void FailedWrite_LeavesNoTempFileBehind()
    {
        ...
        if (OperatingSystem.IsWindows())
        {
            ...
            // through the second write — the real-world case (a sync client
            // or antivirus holding places.json).
        ...
        {
            // Elsewhere file locks are advisory and don't stop a rename, so
            // block the replace a portable way instead: a directory where
            // File.Replace wants to put the backup copy. The temp file has
            // already been written by then, which is what this test needs.
            Directory.CreateDirectory(Path.Combine(dir.Path, "places.bak.json"));
            Assert.ThrowsAny<Exception>(() => storage.Write("new contents"));
        }
