...
| D31: a Favourite checkbox column | A 48 px star column before Alias: a gold filled star for a favourite, a faint outline otherwise, and clicking it toggles favourite (`ToggleFavouriteCommand`, as Ctrl+D). Its header star sorts, favourites first, and toggling while sorted by favourite re-sorts at once, as an open does under a usage sort (D32). A double-click on the star doesn't also open the place | The user asked for it on their second look at the running app. The checkbox was read-only, and a star beside the alias reads better. The star is not focusable, so Tab and the arrow keys still move by row, and Ctrl+D stays the keyboard route. `PlaceViewModel.FavouriteGlyph` and `FavouriteToolTip` are tested |
