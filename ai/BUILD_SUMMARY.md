...
The solution builds cleanly with the compiler, and the service layer is covered by 83 passing unit tests. Everything the UI does on top of that still needs a hands-on pass in Visual Studio on Windows, where it has only been partly exercised so far: dialogs, context menus, double-click, bubble drag-reorder, the grid collapse, window-state restore, the corrupt-file notice, the search box, icon glyphs, and keyboard shortcuts, the save-failure warning and close prompt, the data-folder button, and now the global hotkey and single-instance behaviour.
